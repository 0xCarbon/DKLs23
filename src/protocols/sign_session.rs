...
use crate::protocols::{Abort, Party};

pub struct SignSession<'a> {
    party: &'a Party,
    data: SignData,
    phase1_to_2: Option<(UniqueKeep1to2, BTreeMap<u8, KeepPhase1to2>)>,
    phase2_to_3: Option<(UniqueKeep2to3, BTreeMap<u8, KeepPhase2to3>)>,
...
mod tests {
    ...
    fn test_sign_session_happy_path() {
        ...
        for party_index in executing_parties.clone() {
            ...
            counterparties.retain(|i| *i != party_index);
            all_data.insert(
                party_index,
                SignData {
