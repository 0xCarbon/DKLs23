...
pub struct DkgSession {
    ...
    zero_kept_2to3: Option<BTreeMap<u8, KeepInitZeroSharePhase2to3>>,
    bip_kept_2to3: Option<UniqueKeepDerivationPhase2to3>,
    zero_kept_3to4: Option<BTreeMap<u8, KeepInitZeroSharePhase3to4>>,
    mul_kept_3to4: Option<BTreeMap<u8, KeepInitMulPhase3to4>>,
}

impl DkgSession {
    #[must_use]
    pub fn new(parameters: Parameters, party_index: u8, session_id: Vec<u8>) -> Self {
        DkgSession {
            data: SessionData {
                parameters,
                party_index,
...
mod tests {
    ...
    fn test_dkg_session_full_flow() {
        ...
        let mut sessions: Vec<DkgSession> = (0..parameters.share_count)
            .map(|i| DkgSession::new(parameters.clone(), i + 1, session_id.to_vec()))
            .collect();
        ...
        for (i, session) in sessions.iter_mut().enumerate() {
            ...
            bip_broadcast_2to4.insert(i as u8 + 1, bip_broadcast);
        }
        ...
        for (i, session) in sessions.iter_mut().enumerate() {
            ...
            bip_broadcast_3to4.insert(i as u8 + 1, bip_broadcast);
        }
    ...
    fn test_dkg_session_phase_ordering() {
        ...
        let mut session = DkgSession::new(parameters.clone(), 1, session_id.to_vec());
        ...
        let session = DkgSession::new(parameters.clone(), 1, session_id.to_vec());
        ...
        let mut session = DkgSession::new(parameters, 1, session_id.to_vec());
