...
pub fn commit_point(point: &Point<Secp256k1>) -> (HashOutput, Vec<u8>) {
    let point_as_bytes = point_to_bytes(point);
    commit(&point_as_bytes)
}

pub fn verify_commitment_point(point: &Point<Secp256k1>, commitment: &HashOutput, salt: &[u8]) -> bool {
    let point_as_bytes = point_to_bytes(point);
...
mod tests {
    ...
    fn test_commit_decommit_fail_salt() {
        ...
        let salt1 = rand::thread_rng().gen::<[u8; SECURITY]>(); //We change the salt
        let salt2 = rand::thread_rng().gen::<[u8; SECURITY]>();
        let salt = [salt1,salt2].concat();
        assert!(!(verify_commitment(&msg, &commitment, &salt)));   //The test can fail but with very low probability
    }
